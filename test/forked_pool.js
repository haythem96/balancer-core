...
contract('BPool', async (accounts) => {
    ...
    before(async () => {
        ...
        */
        ...
        // User2 balances
        await weth.mint(user2, toWei('40'), { from: admin });
    ...
    describe('Binding Tokens', () => {
        ...
        it('Fails binding weights and balances outside MIX MAX', async () => {
            ...
            await truffleAssert.reverts(
                pool.bind(DAI, toWei('1000'), toWei('0.49')),
                'ERR_MIN_WEIGHT',
            );
            await truffleAssert.reverts(
        ...
        it('Admin binds tokens', async () => {
            ...
            await pool.bind(WETH, toWei('50'), toWei('0.5'));
            ...
            assert.equal(10.5, fromWei(totalDernomWeight));
            const wethDenormWeight = await pool.getDenormalizedWeight(WETH);
            assert.equal(0.5, fromWei(wethDenormWeight));
            const wethNormWeight = await pool.getNormalizedWeight(WETH);
            assert.equal(0.047619047619047616, fromWei(wethNormWeight));
    ...
    describe('User interactions', () => {
        ...
        */
        ...
        it('getSpotPriceSansFee and getSpotPrice', async () => {
            ...
            assert.equal(20, fromWei(wethPrice));

            const wethPriceFee = await pool.getSpotPrice(DAI, WETH);
            const wethPriceFeeCheck = ((10500 / 5) / (52.5 / 0.5)) * (1 / (1 - 0.003));
        ...
        it('swapExactAmountIn', async () => {
            // 2.5 WETH -> DAI
            const expected = calcOutGivenIn(52.5, 0.5, 10500, 5, 2.5, 0.003);
