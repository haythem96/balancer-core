...
//         // User2 balances
//         await weth.mint(user2, toWei('105303450993235706292649'), { from: admin });
...
//             );
//             await truffleAssert.reverts(
//                 pool.bind(DAI, toWei('1000'), toWei('0.0049')),
//                 'ERR_MIN_WEIGHT',
//             );
//             await truffleAssert.reverts(
...
//             await pool.bind(WETH, toWei('50'), toWei('0.005'));
...
//             assert.equal(10.005, fromWei(totalDernomWeight));
//             const wethDenormWeight = await pool.getDenormalizedWeight(WETH);
//             assert.equal(0.005, fromWei(wethDenormWeight));
//             const wethNormWeight = await pool.getNormalizedWeight(WETH);
//             assert.equal(0.000499750124937531, fromWei(wethNormWeight));
...
//             assert.equal(0.2, fromWei(wethPrice));

//             const wethPriceFee = await pool.getSpotPrice(DAI, WETH);
//             const wethPriceFeeCheck = ((10500 / 5) / (52.5 / 0.005)) * (1 / (1 - 0.003));
//             // 0.200601805416248746
...
//             // 2.5 WETH -> DAI
//             const expected = calcOutGivenIn(52.5, 0.005, 10500, 5, 2.5, 0.003);     //  0.48701695345421937
...
//             const wethPriceFeeCheck = ((10451.409951452899 / 5) / (55 / 0.5)) * (1 / (1 - 0.003));
...
//             assert.equal(0.47619047619047619, fromWei(daiNormWeight));
//         });

//         it('swapExactAmountOut', async () => {
//             // ETH -> 1 MKR
//             // const amountIn = (55 * (((21 / (21 - 1)) ** (5 / 0.5)) - 1)) / (1 - 0.003);
//             const expected = calcInGivenOut(55, 0.5, 21, 5, 1, 0.003);
//             // console.log(expected)

//             // const balanceIn = await pool.getBalance(WETH)
//             // const denormIn = await pool.getDenormalizedWeight(WETH)
//             // const balanceOut = await pool.getBalance(MKR)
//             // const denormOut = await pool.getDenormalizedWeight(MKR)
//             // const tokenAmountIn = 
//             //     await pool.calcInGivenOut(
//             //         balanceIn,
//             //         denormIn,
//             //         balanceOut,
//             //         denormOut,
...
//             //         toWei('0.003')
//             //     )
//             // console.log(tokenAmountIn.toString())

//             // console.log(calcSpotPrice(balanceIn.add(tokenAmountIn), denormIn, balanceOut.sub(1000000000000000000), denormOut, toWei('0.003'))).toString())
            
//             const txr = await pool.swapExactAmountOut(
//                 WETH,
//                 toWei('87303450993235706292649'),
//                 MKR,
//                 toWei('1.0'),
//                 toWei('2630'),
//                 { from: user2 },
